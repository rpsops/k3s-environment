...
**Tooling in toolbox container.** kubectl, helm, flux, cosign, kubeseal, yq, jq live only inside k3s-toolbox. Host gets shims in `~/.local/bin` delegating to `docker exec k3s-toolbox`.
...
**Gitops repos self-register with Flux.** `make k3s-push` publishes to the in-cluster git server; `make k3s-register` creates Flux GitRepository, Kustomization, and policy exceptions. No registration logic in k3s-environment.
